...
how session persistence works via dtach, and known issues with disconnection handling.
...
| **ttyd** (C binary) | WebSocket-to-PTY bridge (no built-in auth — auth handled by Caddy) | 8080 (internal) | claude |
| **claude-session.sh** | Session orchestrator: create or attach dtach session | - | claude |
| **dtach** | Detachable PTY multiplexer (like tmux, but minimal) | - | claude |
...
│                                                  │
│  Started by s6 as claude user with:              │
│    s6-setuidgid claude                           │
│    ttyd -W -p 8080 \                             │
│      -t reconnect=3 \                            │
│      -P 30 \                                     │
...
│  They run on dtach's internal PTY.              │
└──────────────┬───────────────────────────────────┘
               │
               │  dtach's internal PTY
               │
               ▼
┌──────────────────────────────────────────────────┐
...
│  dtach's PTY. Persists across all browser       │
...
```
s6-supervise (ttyd)
└── s6-setuidgid claude ttyd -W -p 8080 ...     ← runs as claude user; auth handled by Caddy
    └── claude-session.sh                        ← ttyd's PTY (pts/0)
        └── dtach -a claude-session              ← attach client (exec'd from claude-session.sh)

dtach -n claude-session claude                  ← dtach server (ppid=1, created detached)
└── claude                                       ← the actual CLI (on dtach's internal PTY, pts/1)
```
...
5. ttyd allocates PTY pts/0, forks: claude-session.sh (already running as claude)
6. claude-session.sh checks: no dtach session exists
7. Runs: dtach -n claude-session claude (creates detached server + claude)
8. dtach server process detaches to ppid=1
9. claude starts inside dtach's internal PTY
10. Runs: exec dtach -a claude-session (attaches client on pts/0)
...
3. claude-session.sh process on pts/0 receives SIGHUP
4. dtach -a (attach client) loses its terminal → exits

Still alive:
  - dtach server process (ppid=1, not on any ttyd PTY)
  - claude process (on dtach's internal PTY)
...
3. Caddy proxies to ttyd; ttyd allocates NEW PTY pts/2, forks new claude-session.sh
4. claude-session.sh checks: dtach session "claude-session" exists and is alive
5. Calls trigger_redraw() in background (will send SIGWINCH to claude)
6. Runs: exec dtach -a claude-session
7. dtach attaches new client to existing server session
...
1. Checks if any dtach session exists
...
4. If TTL expires (default `CLAUDE_SESSION_TTL=1800` = 30 minutes):
   - Kills dtach **server** processes (ppid=1)
...
- A lingering `dtach -a` attach client

**Root cause:** When ttyd closes a PTY on WS disconnect:
- The `dtach -a` client exits but may not be reaped promptly by ttyd
- `claude-session.sh` has already `exec`'d into `dtach -a`, so there's no
...
**Current mitigation:** The `session-cleanup` service kills dtach server processes
after TTL, but does not clean up stale attach clients or zombies.

**Potential fixes:**
1. Add periodic zombie reaping to `session-cleanup` (kill stale `dtach -a` processes)
